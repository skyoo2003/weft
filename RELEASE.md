...
`release.yml` mints a token for [skyoo2003/homebrew-tap](https://github.com/skyoo2003/homebrew-tap) from a GitHub App, because this repository's own `GITHUB_TOKEN` cannot write to another repository and a long-lived PAT would be one more thing to rotate. The App has to be installed on this repository, not only on the tap, and two secrets have to exist here:

```bash
gh secret set TAP_APP_ID -R skyoo2003/weft
gh secret set TAP_APP_PRIVATE_KEY -R skyoo2003/weft < tap-app.private-key.pem
```

Step 4 refuses in seconds when they are missing, before anything is built, and re-running the workflow after setting them is safe. That is the only reason this is a note rather than a step: getting it wrong costs a re-run, not a tag.
...
- [`release.yml`](.github/workflows/release.yml) refuses outright if `changes/<tag>.md` is missing or if the tap credentials are absent, then runs GoReleaser: cross-compiled archives of `cmd/weft` and `cmd/weftd`, `checksums.txt`, an SBOM per archive, the GitHub release itself with `changes/<tag>.md` as its notes, a container image on ghcr.io, and a cask committed to [skyoo2003/homebrew-tap](https://github.com/skyoo2003/homebrew-tap).

```bash
gh run list --workflow=release.yml --limit 1
gh release view v0.1.0
GOPROXY=proxy.golang.org go list -m github.com/skyoo2003/weft@v0.1.0
docker buildx imagetools inspect ghcr.io/skyoo2003/weft:v0.1.0
brew update && brew info --cask skyoo2003/tap/weft
```

The `go list` line is the one that matters for a library: it is what a user's `go get` will do. The two below it are the only way to find out that the other half of the job worked, because GoReleaser publishes the release page before the image and the cask — a green release page is not evidence that either exists.
...
**Backfilling a tag.** [`backfill-image.yml`](.github/workflows/backfill-image.yml) publishes the image for a tag that already exists — one cut before the image did, or one whose release job died between the archives and the push. It runs only from the Actions tab, and that is the point: it is a recovery path, and a recovery path that fires on its own is one nobody notices firing. It builds with `go build` and `docker buildx` and cannot write to the release page at all, so aiming it at the wrong tag costs a wasted image and nothing else.
