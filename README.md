...
| No query language, and no range operator | Queries are built through the Go API. `pkg/query` covers the boolean shell (`Must`, `MustNot`), term patterns (`Glob` — prefix, suffix, wildcard, character class) and exact phrases; `text.NewField` covers field restriction. There is still **no string syntax to parse into them**, no numeric or date range, and no edit-distance matching. Ranges want index support this format does not have; the parser is worth writing once there is something stable to parse into. |
