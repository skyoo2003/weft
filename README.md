...
| No phrase or proximity search | `Posting` carries a term's document and frequency, **not its positions**, so no exact phrase can be decided from the index. A scorer can decide one from `Document.Text` — wrap another scorer and filter its candidates, at one record decode each. A position index is a format change: [FORMAT §8](docs/FORMAT.md). |
...
| A constraint needs a `Fuser` | Rank fusion is a union of votes, so a scorer that returns only matching documents ranks rather than filters, and what it refused returns on another stream's vote with no error. Excluding means writing a `Fuser`: [ADOPTION §8](docs/ADOPTION.md), `ExampleFuser`. |
| No query language | Queries are built through the Go API — terms, a vector, graph seeds. There is no string syntax, and no boolean, phrase, field or range operator; each of those is a scorer plus a `Fuser` you write. |
