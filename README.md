...
| 16 | Query expressiveness — boolean, pattern, phrase | ✅ `pkg/query` ships the `Fuser` every adopter had to write for themselves — `Must`, `MustNot`, composable and indexed by stream position — plus `Glob` for prefix/suffix/wildcard terms and `Phrase` for exact runs. No format change, no new dependency, and `go list -deps ./pkg/query` names no scorer. Still missing: a string syntax, field restriction, numeric ranges, fuzzy matching |
...
pkg/
  ...
  query/           Must/MustNot restriction fusers, Glob term patterns,
                   Phrase — imports engine and nothing else
  scorer/
...
| No field restriction | A `Document` has one `Text` and the index has no notion of a field, so "title only" is a convention you keep on your side: a table keyed by `Document.Key`, or a second index. Expressible, and nothing in weft helps: [ADOPTION §8](docs/ADOPTION.md). |
...
| No query language, and no field or range operator | Queries are built through the Go API. `pkg/query` covers the boolean shell (`Must`, `MustNot`), term patterns (`Glob` — prefix, suffix, wildcard, character class) and exact phrases. There is still **no string syntax to parse into them**, no field restriction, no numeric or date range, and no edit-distance matching. Field restriction and ranges want index support this format does not have; the parser is worth writing once there is something stable to parse into. |
