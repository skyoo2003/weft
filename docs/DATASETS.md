...
**Answer: yes** — but no single dataset works off the shelf. It has to be **human-made labels plus a link graph joined from a separate source** (§1). Milestone 2's scope is unaffected (§4).

---

## 1. Disqualification criterion

The graph scorer converts hop distance from a seed into `1/(1+hops)`.

**A dataset whose labels were derived from the same link structure is unusable.** The scorer reproduces the label-generation rule, so a higher nDCG is guaranteed and means nothing.

Same failure mode as the seed echo in [FINDINGS §2.3](FINDINGS.md): there one text vote was counted twice, here the answer key would be visible during the exam.
...
| HotpotQA | ❌ as primary | Labels are human, but annotators were shown **link-connected paragraph pairs** to write questions from, so gold pairs are hyperlink edges by construction. Usable only as an upper bound: no improvement here means the implementation is broken |
| ClueWeb09 + TREC Web Track | ⚠️ blocked | Satisfies everything, and is gated. See below |
| **TREC-COVID ⋈ Semantic Scholar** | ✅ recommended | §3 |
| DBpedia-Entity v2 | ✅ fallback | 467 queries, 49,280 judgments on a 3-point scale, crowdsourced with expert adjudication. Labels human, graph is DBpedia's RDF structure. Costs more setup: 4.6M entities and a separate dump |

**ClueWeb09** has 70,575 human graded judgments over 200 topics plus a complete web graph (454M outlinks, 3 GB uncompressed) independent of the labels. The qrels are free from trec.nist.gov; the collection needs a licence and is 5 TB compressed. It is the end goal once scale allows.
...
That is very nearly the function the graph scorer computes, and all 169,756 judgments in the original release are automatically extracted rather than human-assessed.

Recorded explicitly because the attractive size makes it likely to be picked up again.
...
3. **Expect no link structure from the tooling.** In the `ir_datasets` catalogue, `links` and `citations` mean documentation links and bibliographic entries, not inter-document edges. BEIR's `corpus.jsonl` carries only `_id`, `title` and `text`. The graph must be joined in.
...
- **Keep `Document.Links` keyed by document key.** The recommended path is exactly "join an external citation graph by document id", and many targets fall outside the corpus and remain dangling — already handled by design (`TestDanglingLinksAreIgnored`). A `DocID` adjacency list would have blocked this path.
